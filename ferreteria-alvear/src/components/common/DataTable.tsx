    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          {subtitle && <p className="text-text-secondary mt-1 text-sm sm:text-base">{subtitle}</p>}
        </div>
        {actions && <div className="flex gap-2 sm:gap-3 w-full sm:w-auto">{actions}</div>}
      </div>

      <div className="border border-border rounded-lg shadow-lg bg-surface">
        {onSearch && (
          <div className="p-4 lg:p-6 border-b border-border">
            <div className="relative w-full sm:max-w-md">
              <Search01Icon size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-text-tertiary" />
              <Input
                placeholder={searchPlaceholder}
                onChange={(e) => onSearch(e.target.value)}
                className="pl-10 bg-background border-border text-text placeholder:text-text-tertiary"
              />
            </div>
          </div>
        )}
              ...
              {loading ? (
                ...
                data.map((item) => (
                  <tr
                    ...
                    className={`border-b border-border hover:bg-background-secondary/30 transition-colors ${
        ...
        {totalLabel && (
          <div className="px-4 lg:px-6 pb-4 lg:pb-6">
            <p className="text-sm text-text-secondary uppercase tracking-wide">{totalLabel}</p>
          </div>
        )}
      </div>
    </div>
